    ...
    <form wire:submit="updateUser" class="w-full px-6 pt-6 pb-3" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="space-y-4 order-2 md:order-1">
                ...
                <div class="relative mb-5">
                    ...
                    <select 
                        id="gender" 
                        wire:model="gender"
                        class="appearance-none w-full p-3 bg-white border border-gray-200 rounded-lg shadow-sm 
                               text-[13px] text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400 
                               focus:border-blue-400 transition-all duration-200 ease-in-out pr-10 
            ...
            <div class="space-y-4 order-3 md:order-2">
                ...
                {{-- Username --}}
                <div class="relative mb-5">
                    <label for="username" class="block text-[14px] text-gray-600 mb-2 font-medium font-secondary">Username</label>
                    <input 
                        type="text" 
                        id="username"
                        wire:model="username"
                        ...
                        class="w-full p-3 bg-white border border-gray-200 text-[13px] rounded-lg shadow-sm 
                               focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400 
                               transition-all duration-200 ease-in-out placeholder:text-gray-400 
                               @error('username') border border-red-500 focus:ring-red-300 @enderror"
                        style="font-family: 'Inter', sans-serif;">
                    @error('username')
                        <span class="absolute left-0 top-full mt-1 text-red-500 text-[11px] font-secondary">{{ $message }}</span>
                    @enderror
                ...
                <div class="mb-10 md:mb-5 lg:mb-5 relative">
                    ...
                    <input 
                        ...
                        class="w-full p-3 bg-white border border-gray-200 text-[13px] rounded-lg shadow-sm 
                               focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400 
                               transition-all duration-200 ease-in-out placeholder:text-gray-400 
                               @error('password') border border-red-500 focus:ring-red-300 @enderror"
                        style="font-family: 'Inter', sans-serif;">
                    @error('password')
                        <span class="absolute left-0 top-full mt-1 text-red-500 text-[11px] font-secondary">{{ $message }}</span>
                    @enderror
                ...
                <button type="submit"
                    class="w-full py-3 mb-5 bg-[#3B82F6] text-[#F9FAFB] text-[13px] font-medium rounded-md 
                        hover:bg-[#1E40AF] transition duration-200 block lg:hidden md:hidden cursor-pointer"
                    style="font-family: 'Inter', sans-serif;">
                    Update
                </button>
            </div>
            ...
            <div class="flex flex-col items-center justify-between order-1 md:order-3">
                <div x-data="{ preview: null }" class="flex-grow flex flex-col items-center justify-center mb-6 md:mb-0">
                    <div class="relative w-32 h-32">
                        ...
                        <template x-if="!preview">
                            <div>
                                @if($currentAvatar)
                                    <img src="{{ '/static/storage/avatars/' . $currentAvatar }}" alt="{{ $first_name . ' ' . $last_name }}" 
                                         class="absolute inset-0 w-full h-full object-cover rounded-full border-4 border-blue-100">
                                @else
