<div>
    <form wire:submit="update" class="w-full px-6 py-2 lg:py-6" enctype="multipart/form-data">
        ...
        <div class="relative mb-8">
            ...
            <div class="flex flex-wrap gap-6 mt-2">
                @foreach ($all_folder_types as $folder)
                    <label class="flex items-center space-x-2 cursor-pointer">
                        <input type="checkbox"
                            wire:model="folders"
                            value="{{ $folder['id'] }}"
                            class="w-5 h-5 accent-blue-500 focus:ring-blue-400 border-gray-300 rounded-md"
                            @if(in_array($folder['id'], $folders)) checked @endif>
                        <span class="text-[14px] text-gray-700" style="font-family: 'Inter', sans-serif;">
                            {{ $folder['name'] }}
                        </span>
                    </label>
                @endforeach
        ...
        <button type="submit"
            class="w-full py-3 mb-4 lg:mb-0 bg-[#3B82F6] text-[#F9FAFB] text-[14px] font-medium rounded-md 
                hover:bg-[#1E40AF] transition duration-200 block cursor-pointer"
            style="font-family: 'Inter', sans-serif;">
            Update
        </button>
    </form>
